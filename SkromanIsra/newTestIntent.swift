...
//                            let switches = self.createSwitches(from: deviceState, buttonDetails: self.filteredButtonDetails )
//                           
...
//        }
//    }
//    func printSwitches(_ switches: [SwitchItem], buttonDetails: [ButtonDetails]) {
//        print("\n📱 Total switches: \(switches.count)")
//
//        // Create a lookup dictionary for fast access
//        let buttonMap = Dictionary(uniqueKeysWithValues: buttonDetails.map { ($0.buttonNo, $0) })
//
//        for item in switches {
//            let buttonInfo = buttonMap[item.switchIndex]
//
//            switch item.type {
//            case .light:
//                print("""
//                🔆 Light \(item.switchIndex): isOn=\(item.isOn), childLocked=\(item.isChildLocked), uniqueID=\(item.uniqueID)
//                ↪️  Button: \(buttonInfo?.buttonName ?? "N/A"), icon: \(buttonInfo?.buttonIconName ?? "N/A"), controlName: \(buttonInfo?.buttonControlName ?? "N/A")
//                """)
//
//            case .fan:
//                print("""
//                🌀 Fan \(item.switchIndex): isOn=\(item.isOn), speed=\(item.speed ?? "-"), childLocked=\(item.isChildLocked), uniqueID=\(item.uniqueID)
//                ↪️  Button: \(buttonInfo?.buttonName ?? "N/A"), icon: \(buttonInfo?.buttonIconName ?? "N/A"), controlName: \(buttonInfo?.buttonControlName ?? "N/A")
//                """)
//
//            case .master:
//                let masterInfo = buttonMap[0] // assuming buttonNo = 0 for master
//                print("""
//                🎛️ Master: isOn=\(item.isOn), childLocked=\(item.isChildLocked), uniqueID=\(item.uniqueID)
//                ↪️  Button: \(masterInfo?.buttonName ?? "N/A"), icon: \(masterInfo?.buttonIconName ?? "N/A"), controlName: \(masterInfo?.buttonControlName ?? "N/A")
//                """)
//            }
//        }
//
//        print("\n🧾 Full Switch Array Dump:\n\(switches)\n")
//    }
//
...
//                    buttonDetail: buttonIndex < buttonDetails.count ? buttonDetails[buttonIndex] : nil // ✅ append
//                )
//                switches.append(switchItem)
//                buttonIndex += 1
//                print("switchItem at light \(switchItem.buttonDetail)")
//            }
//        }
//
//        // FANS
...
//            switches.append(switchItem)
//            buttonIndex += 1
//            print("switchItem at fan \(switchItem.buttonDetail)")
//        }
//
//        // MASTER
...
//
//        if let indexPath = ShButtonCollectionView.indexPathForItem(at: point) {
//            let buttonLabel = buttonItems[indexPath.item]
//            print("🟡 Long-pressed cell at index: \(indexPath.item)")
//            let btnNumber = indexPath.item + 1
//            print("no=\(indexPath.item)")
//            
//            let buttonNumber = Int(buttonLabel.dropFirst()) ?? -1
//            let controlType = String(buttonLabel.prefix(1))
//
//            let detail = mappedValues.first {
//                $0["cNm"] == controlType && $0["d_no"] == String(buttonNumber)
//            }
//           
//            print("detail at \(detail)")
//            let type = detail?["cNm"] ?? ""
//            let status = detail?["L_state"] ?? ""
//            let fanstste  = currentDeviceState?.fanState
//            let selectedButtonItem = (name: buttonLabel, type: type, status: status)
//            let matchedButtonDetails = buttonDetails.first { $0.buttonNo == buttonNumber }
//           print(fanstste)
//            let storyboard = UIStoryboard(name: "Main", bundle: nil)
//            if let editVC = storyboard.instantiateViewController(withIdentifier: "EditButtonViewController") as? EditButtonViewController {
//                
//                editVC.selectedButtonItem = selectedButtonItem
//                editVC.devices = self.devices
//                editVC.devicestate = self.receivedDeviceStates
//                editVC.selectedButtonDetail = detail
//                editVC.isFromLongPress = true
//                editVC.selectedBtnNumber = btnNumber
...
//                        presenter = presented
//                    }
//                    presenter.present(editVC, animated: true, completion: nil)
//                }
//            }
//        }
//    }
...
//          
//            print ("buttonText atttt\(deviceUniqueId )")
//            
//            
...
//            let qState = mappedValues.first(where: { $0["cNm"] == "Q" })?["L_state"] ?? ""
//            let yState = mappedValues.first(where: { $0["cNm"] == "Y" })?["L_state"] ?? ""
//            let fspeed = mappedValues.first(where: { $0["c_f"] == "F" })?["F_speed"] ?? ""
//            let dimSpeed = mappedValues.first(where: { $0["c_l"] == "L" })?["L_speed"] ?? ""
//            
//            
...
//
//
//extension SwitchItem: CustomStringConvertible {
//    var description: String {
//        let speedStr = speed ?? "nil"
//        return "SwitchItem(name: \"\(name)\", type: \"\(type.rawValue)\", switchIndex: \(switchIndex), isOn: \(isOn), isChildLocked: \(isChildLocked), speed: \(speedStr))"
//    }
//}
//
...
//                self.FanSpeed(speedValue: sliderValue)
//            } else if self.currentSliderControlType == "L", let buttonNo = self.currentButtonNo {
...
//                return !invalidFirstChars.contains(firstChar)
//            }
//            print("✅ Filtered buttons: \(buttonDetails)")
//        }
//    }
//
//
...
//        let validPrefixSet: Set<Character> = ["L", "O", "C", "D", "Q", "Y"]
//        if let firstChar = buttonText.first, validPrefixSet.contains(firstChar),
//           index < buttonDetails.count {
//            self.buttonName.text = buttonDetails[index].buttonName
//        } else {
//            self.buttonName.text = buttonText
//            print("buttonText\(buttonText)")
//        }
//
//           self.isHidden = false
//           self.deviceVew.backgroundColor = UIColor.clear
//           
//          
//        if let deviceUniqueId = deviceUniqueId {
//            fetchButtonsDetails(SelectedUniqueUid: deviceUniqueId)
//               print("deviceUniqueId inside configure: \(deviceUniqueId)")
//           } else {
//               print("deviceUniqueId is nil in configure")
//           }
//        
//        
...
//           let qState = mappedValue["cNm"] == "Q" ? mappedValue["L_state"] ?? "" : ""
//           let yState = mappedValue["cNm"] == "Y" ? mappedValue["L_state"] ?? "" : ""
//           let lspeed = mappedValue["c_l"] == "L" ? mappedValue["L_speed"] ?? "" : ""
//           let fspeed = mappedValue["c_f"] == "F" ? mappedValue["F_speed"] ?? "" : ""
//           
//          
//           
...
//           let fanSpeed = Int(mappedValue["FSpeed"] ?? "1") ?? 1
//           let dimmingLevel = Int(mappedValue["L_speed"] ?? "1") ?? 1
//           
//           let activeColor = UIColor(hex: "#FAEDCB")
//           let inactiveColor = UIColor(hex: "#FFFFFF")
//           
//        if buttonText.starts(with: "L") {
//            self.buttonImageView.image = getIconImage(defaultName: "bulb", index: index)
//            self.deviceVew.backgroundColor = (lState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "O") {
//            self.buttonImageView.image = getIconImage(defaultName: "curtains_Open", index: index)
//            self.deviceVew.backgroundColor = (oState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "C") {
//            self.buttonImageView.image = getIconImage(defaultName: "curtains_close", index: index)
//            self.deviceVew.backgroundColor = (cState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "Y") {
//            self.buttonImageView.image = getIconImage(defaultName: "curtains_Open", index: index)
//            self.deviceVew.backgroundColor = (oState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "Q") {
//            self.buttonImageView.image = getIconImage(defaultName: "curtains_close", index: index)
//            self.deviceVew.backgroundColor = (cState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "D") {
//            self.buttonImageView.image = getIconImage(defaultName: "lock-2", index: index)
//            self.deviceVew.backgroundColor = (cState == "1") ? activeColor : inactiveColor
//        } else if buttonText.starts(with: "F") {
//            self.buttonImageView.image = getIconImage(defaultName: "ceiling-fan", index: index)
//            self.deviceVew.backgroundColor = (fState == "1") ? activeColor : inactiveColor
//            FanAndDimmingSlider.isHidden = (fState != "1")
//            FanAndDimmingSlider.minimumValue = 1
//            FanAndDimmingSlider.maximumValue = 4
//            FanAndDimmingSlider.value = Float(fanSpeed)
//            sliderValueLabel.text = "\(fanSpeed)"
//        } else if buttonText == "Master" {
//            self.buttonImageView.image = getIconImage(defaultName: "AppIcon1", index: index)
//            let masterMapped = mappedValues.first(where: { $0["Master"] != nil })
//            let masterState = masterMapped?["Master"] ?? "0"
//            self.deviceVew.backgroundColor = (masterState == "1") ? activeColor : inactiveColor
//        } else {
//            self.buttonImageView.image = nil
//        }
//
//           if dimState == "1" {
//                   self.dimStateImage.image = UIImage(named: "brightness-2")
//                   self.dimStateImage.isHidden = false
//               
//               } else {
//                   self.dimStateImage.isHidden = true
//                   
//               }
//           FanAndDimmingSlider.isHidden = true
//        if fState == "1" {
//            FanAndDimmingSlider.isHidden = false
//            FanAndDimmingSlider.tintColor = UIColor.systemBlue
//            FanAndDimmingSlider.minimumValue = 1
//            FanAndDimmingSlider.maximumValue = 4
//            FanAndDimmingSlider.value = Float(fanSpeed)
//            sliderValueLabel.text = "\(fanSpeed)"
...
//            currentButtonNo = index + 1
//        }
//        if dimState == "1" {
//            dimStateImage.image = UIImage(named: "brightness-2")
//            dimStateImage.isHidden = false
//            FanAndDimmingSlider.isHidden = false
//            FanAndDimmingSlider.tintColor = UIColor.systemOrange
//            FanAndDimmingSlider.minimumValue = 1
//            FanAndDimmingSlider.maximumValue = 7
//            FanAndDimmingSlider.value = Float(dimmingLevel)
//            sliderValueLabel.text = "\(dimmingLevel)"
//            
//            currentSliderControlType = "L"
//            currentButtonNo = index + 1
//        }
//  if childLock == "1" {
//                   self.lockImageView.image = UIImage(named: "lockIcon")
...
//        guard index < buttonDetails.count else {
//            return UIImage(named: defaultName)
//        }
//
//        let iconName = buttonDetails[index].buttonIconName
//        let controlName = buttonDetails[index].buttonControlName
//
//        // List of disallowed curtain icons for control type "L"
//        let disallowedIconsForL: Set<String> = [
//            "curtains_Open",
//            "curtains-2",
//            "curtains_close2",
//            "curtains_close"
//        ]
//
//        // If control is "L" and the icon is a disallowed curtain icon, fallback to default
//        if controlName == "L", disallowedIconsForL.contains(iconName) {
//            return UIImage(named: defaultName)
//        }
//
//        // If icon is "Unknown", fallback to default
//        if iconName == "Unknown" {
//            return UIImage(named: defaultName)
//        }
//
//        // If image with given name exists, return it
